...
def user_input_audio(audio):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a english grammar specialist to check if there are any grammatical and punctuation errors in the text generated by the audio provided by the user. You have to display the original text generated by the audio and highlight the grammatical and punctuation errors in the original text and finally correct the grammatical and punctuation errors and show the correct text. The highlighted errors should display the grammatical errors."  ),
        ("user", "{input}")
    ])
